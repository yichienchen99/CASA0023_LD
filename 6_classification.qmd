...
-   Still confused about which classifier to choose when performing image classification, although knowing reference to literature on relevant themes might hint the direction and performing/comparing different classifiers might just be within several lines of code in GEE. Does the accuracy means everything? Choosing the one with highest accuracy might not be reproducible in the sense that different regions/time of year/giving new data could yield varying results?
...
-   Image classification: DN/reflectance of imagery \> being divided based on similarities/closeness to each other using different forms of classifiers/algorithms \> revealing/predicting the LULC of the Earth surface \> testing accuracy \> being used for further analysis / practical applications in assisting decision making
